...
class Notice_model extends CI_Model
{
	...
	public function getNoticeList($data)
	{
		...
		if ( isset($data['searchLan']) && $data['searchLan'] !== 'all' ) {
			$where .= " and B.N_LANGUAGE = '".$data['searchLan']."'";
		}
		...
		$query = sprintf("
            ...
            FROM 
				P_BRANCH A
                INNER JOIN ".getenv('DATABASE_NAME').".P_NOTICE AS B ON A.BRANCH = B.N_BRANCH
        	WHERE
        	    B.N_DEL_YN = 'N'
				%s
        	    %s
...
//		echo $query;
	...
	public function delete_notice($updateData, $SEQ)
	{
		$this->db->where('SEQ', $SEQ);
		$result = $this->db->update(getenv('DATABASE_NAME').'.P_NOTICE', $updateData);
	...
	public function getNoticeInfo($SEQ = null)
	{
		...
		$query = sprintf("
            ...
            FROM 
                ".getenv('DATABASE_NAME').".P_NOTICE AS A
        	WHERE
        	    A.N_DEL_YN = 'N'
        	    %s
        	ORDER BY 
...
//		echo $query;
	...
	{
		$result = $this->db->insert(getenv('DATABASE_NAME').'.P_NOTICE', $insertData);
