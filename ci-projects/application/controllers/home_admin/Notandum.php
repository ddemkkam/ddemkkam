...
{
	public $menu = 'menu5';
	public $nav = array('navTitle' => '시술전&후 주의사항', 'navLink1' => '/home_admin/notandum', 'navTitle2' => '', 'navLink2' => '');
	...
	public function __construct()
	{
		...
		$this->load->model('/home_admin/Notandum_model', 'Notandum_model');
	...
	public function index($B_BRANCH = null, $B_LAN = null)
	{
		...
		$result['branchListData'] = $this->Notandum_model->getNotandumList($data);
		...
		$this->layout->view('/admin/content/notandum/notandum', $result, true);
	}
	...
	public function delete()
	{
		...
		$res = $this->Notandum_model->delete_notandum($updateData, $data['SEQ']);
		$updateData['SEQ'] = $data['SEQ'];

		if ( $res ) {
			$this->HistoryData = new HistoryData();
			$this->HistoryData->insertHistoryData('DELETE', 'ppeum_homepage.P_NOTANDUM', json_encode($updateData, JSON_UNESCAPED_UNICODE));
	...
	public function modifynotandum($B_BRANCH = null, $B_LAN = null, $SEQ = null)
	{
		$data['searchBranch'] = $B_BRANCH;
		$data['searchLan'] = $B_LAN;

		if ( isset($SEQ) && $SEQ !== '' ) {
			$info = $this->Notandum_model->getNoticeInfo($SEQ);
			$result['info'] = $info[0];
		}
		...
		$this->layout->setLayout("/admin/layouts/layout");
		$this->layout->view('/admin/content/notandum/notandum_register', $result, true);
	}
	...
	public function save()
	{
		...
		$data['N_SUB_TITLE'] = $this->input->post('N_SUB_TITLE');
		...
		if ( isset($data['SEQ']) && $data['SEQ'] !== '' ) {
			...
			$updateData['N_SUB_TITLE'] = $data['N_SUB_TITLE'];
			$updateData['N_TYPE'] = $data['N_TYPE'];
			$updateData['N_CONTEXT'] = $data['N_CONTEXT'];
			$updateData['N_COUNT'] = $data['N_COUNT'];
			$updateData['N_REG_ID'] = $data['N_REG_ID'];

			$res = $this->Notandum_model->update_notice($updateData, $data['SEQ']);

			if ( $res ) {
				$this->HistoryData = new HistoryData();
				$this->HistoryData->insertHistoryData('UPDATE', 'ppeum_homepage.P_NOTANDUM', json_encode($data, JSON_UNESCAPED_UNICODE));

				echo "<script>alert('수정 하였습니다.'); location.replace('/home_admin/notandum/index/".$data['N_BRANCH']."/".$data['N_LANGUAGE']."');</script>";
			} else {
			...
			$insertData['N_SUB_TITLE'] = $data['N_SUB_TITLE'];
			$insertData['N_TYPE'] = $data['N_TYPE'];
			$insertData['N_CONTEXT'] = $data['N_CONTEXT'];
			$insertData['N_COUNT'] = $data['N_COUNT'];
			$insertData['N_REG_ID'] = $data['N_REG_ID'];

			$res = $this->Notandum_model->insert_notice($insertData);

			if ( $res ) {
				$this->HistoryData = new HistoryData();
				$this->HistoryData->insertHistoryData('INSERT', 'ppeum_homepage.P_NOTICE', json_encode($insertData, JSON_UNESCAPED_UNICODE));

				echo "<script>alert('저장 하였습니다.'); location.replace('/home_admin/notandum/index/".$data['N_BRANCH']."/".$data['N_LANGUAGE']."');</script>";
			} else {
