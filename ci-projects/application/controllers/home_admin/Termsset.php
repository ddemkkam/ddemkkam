...
class Termsset extends CI_Controller
{
	public $menu = 'menu1';
	public $nav = array('navTitle' => '이용약관 설정', 'navLink1' => '/home_admin/termsSet', 'navTitle2' => '', 'navLink2' => '');
	...
	public function index($B_BRANCH = null, $B_LAN = null)
	{
		...
		$this->layout->view('/admin/content/tremsset/tremsset', $result, true);
	}
	...
	public function modifyuseset($B_BRANCH = null, $B_LAN = null, $SEQ = null)
	{
		$data['searchBranch'] = $B_BRANCH;
		$data['searchLan'] = $B_LAN;
		...
		$this->layout->setLayout("/admin/layouts/layout");
		$this->layout->view('/admin/content/tremsset/trems_regist', $result, true);
	}
	...
	public function saveTermsSet()
	{
		...
		if ( $res ) {
			...
			echo "<script>alert('저장 하였습니다.'); location.replace('/home_admin/termsset/modifyuseset/".$data['T_BRANCH']."/".$data['T_LAN']."');</script>";
		} else {
