			...
			<select class="form-control" id="category1" onchange="LCategoryChange();">
				...
				if (isset($list)) {
					foreach ($list as $key => $value) {?>
						<option value="<?= str_replace(" ","",preg_replace("/[ #\&\+\-%@=\/\\\:;,\.'\"\^`~\_|\!\?\*$#<>()\[\]\{\}]/i", "", $key)) ?>" >
							<?= $key ?>
						</option>
					<?
					}
				...
				if (isset($list)) {
					foreach ($list as $key => $value) {
						foreach ($value as $key2 => $value2) { ?>
							<option class="sc2 sc_<?= str_replace(" ","",preg_replace("/[ #\&\+\-%@=\/\\\:;,\.'\"\^`~\_|\!\?\*$#<>()\[\]\{\}]/i", "", $key)) ?>" value="sc_<?= str_replace(" ","",preg_replace("/[ #\&\+\-%@=\/\\\:;,\.'\"\^`~\_|\!\?\*$#<>()\[\]\{\}]/i", "", $key2)) ?>" style="display: none;"><?= $key2 ?></option>
						<?
						}
					}
				} ?>
		...
		foreach ($list as $key => $value) {
			foreach ($value as $key2 => $value2) {
				foreach ($value2 as $key3 => $item) { ?>
					<tr class="p_p p_sc_<?= str_replace(" ","",preg_replace("/[ #\&\+\-%@=\/\\\:;,\.'\"\^`~\_|\!\?\*$#<>()\[\]\{\}]/i", "", $key2)) ?>" style="display: none;">
