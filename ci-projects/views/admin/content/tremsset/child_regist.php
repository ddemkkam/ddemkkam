...
<div class="row">
	<div class="col-xs-10">
		<div class="box">
			<div class="box-body">
				<form id="event_frm" action="/home_admin/child/saveTermsSet" method="POST" enctype="multipart/form-data">
					...
					<table id="division_table" class="table table-bordered">
						...
						<?if ( $TYPE === 'child' ) {?>
							<tr>
								<th class="active text-center">파일</th>
								<td>
									<iframe
										src="/fileUpload/uploadView?id=child&cnt=1&viewId=viewId"
										frameborder="0"
										style="width: 100%; height: 200px;"
									></iframe>
									<div id="viewId">
										<a href="<?=isset($info['T_CONTEXT']) ? $info['T_CONTEXT'] : '' ?>">
											<?=isset($info['T_CONTEXT']) ? str_replace("/data/child/", "", $info['T_CONTEXT']) : '' ?>
										</a>
										<input type='button' value='삭제' onclick='deleteZip()'>
									</div>

									<!--									<input type="button" class="btn btn-success" value="업로드" onclick="file_upload_open();" />-->
								</td>
							</tr>
						<?} else {?>
							<tr>
