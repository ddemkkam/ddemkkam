...
<!--								<option value="--><?php //=$row['BRANCH']?><!--"-->
<!--									--><?php //=isset($searchBranch) && $row['BRANCH'] === $searchBranch ? 'selected' : '' ?>
<!--								>-->
<!--									--><?php //=$row['BRANCH_NAME']?>
<!--								</option>-->
<!--							--><?//}?>
...
<!--			</div>-->
		...
		<div class="col-xs-12 right" style="padding-right: 0px;">
			<a href="/home_admin/media/modifymedia/"
			   class="btn btn-dark"
			   id="add_btn"
			   style="float:right"
			>
	...
	<div class="col-xs-10">
		<div class="box">
			<div class="box-header">
				<h3 class="box-title" id="total_count">Total : <?=isset($branchListDataTotal) ? count($branchListDataTotal) : '0'?></h3>
			</div>
			<div class="box-body">
					...
					<?if ( isset($branchListData) ) {?>
						<?foreach ( $branchListData as $index => $row ) {?>
							...
							<tr class="data_list">
								...
								<td class='text-center'>
									<a href="/home_admin/media/modifymedia/<?=$row['SEQ']?>">
										<button type='button' class='btn btn-warning'>수정</button>&nbsp;
									</a>
									<button type='button' class='btn btn-default' onclick="delete_media('<?=$row['SEQ'] ?>')">삭제</button>&nbsp;
								</td>
		...
		$.confirm({
			...
			buttons: {
				tryAgain: {
					...
					btnClass: 'btn-blue',
					action: function(){
						sendAjaxHtml("/home_admin/media/delete", data, 'POST', 'add_ajax_result');
					}
				},
				close: function () {
				}
