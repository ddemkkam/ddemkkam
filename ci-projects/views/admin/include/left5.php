	...
	<section class="sidebar">
		...
		<ul class="sidebar-menu" data-widget="tree">
			<?if ( $this->session->userdata('ADMIN_LEVEL') >= '80' ) {?>
				<li class="<?=strpos($_SERVER['REQUEST_URI'], '/home_admin/notice/') !== false ? 'active menu-open' : ''?>">
					<a href="/home_admin/notice/index/<?=isset($BASEBRANCH) ? $BASEBRANCH.'/' : ''?><?=isset($BASELAN) ? $BASELAN.'/' : ''?>">
						<i class="fa fa-circle-o <?=strpos($_SERVER['REQUEST_URI'], '/home_admin/notice') !== false ? 'text-aqua' : ''?>"></i>
						<span>공지사항</span>
					</a>
				</li>
			<?}?>
			<li class="<?=strpos($_SERVER['REQUEST_URI'], '/home_admin/notandum/') !== false ? 'active menu-open' : ''?>">
				<a href="/home_admin/notandum/index/<?=isset($BASEBRANCH) ? $BASEBRANCH.'/' : ''?><?=isset($BASELAN) ? $BASELAN.'/' : ''?>">
					<i class="fa fa-circle-o <?=strpos($_SERVER['REQUEST_URI'], '/home_admin/notandum') !== false ? 'text-aqua' : ''?>"></i>
					<span>시술전&후 주의사항</span>
				</a>
			</li>
