$(function() {
	...
	$('.btn_add').text('Lưu dịch vụ');	//시술 담기
	$('.btn_view').text('Tìm hiểu chi tiết');	//상세 보기

	$('.hipassSwitch').html('Nhanh chóng, không cần chờ đợi/tư vấn! <em>Xem dịch vụ Hi Pass</em>');	//대기/상담없이 빠르게! 하이패스 시술보기
	...
	var validator = $( "#q_form" ).validate({
		...
		messages: {
			tblStrField2: "Lựa chọn ngày hẹn",	//희망 예약일을 선택해주세요.
			tblStrField3: "Lựa chọn giờ hẹn", //희망 예약시간을 선택해주세요.
			select: "지점을 선택해주세요",
			name: "Tên", //이름을 입력해주세요.
			country: "Vui lòng nhập quốc tịch của bạn ",	//국적을 입력해주세요.
			etcDirectOpt: "바로예약 상담 항목을 선택해주세요.",
			phoneNumber: {
				required: "Vui lòng điền thông tin liên lạc", //연락처를 입력해주세요
				minlength: "Vui lòng điền thông tin liên lạc",
				maxlength: "Vui lòng điền thông tin liên lạc"
			},
			email: {
				required: "Email", //이메일 주소를 입력하세요
				email: "Vui lòng nhập chính xác địa chỉ email" //정확한 이메일 주소를 입력하세요.
			},
			messengerId: "Xin vui lòng nhập số điện thoại hoặc ID giúp chúng tôi tìm được bạn qua Messenger ",	//페이스북 메신저 계정을 검색할 수 있는 ID 혹은 전화번호를 입력해주세요
			tblStrField: "진료구분을 선택해주세요.",
			agree: "Đồng ý với các điều khoản và điều kiện để sử dụng dịch vụ.", //서비스 이용을 위해 약관에 동의하십시오.
			smspush: "Đồng ý với các điều khoản và điều kiện để sử dụng dịch vụ." // 서비스 이용을 위해 약관에 동의하십시오.
		},
		submitHandler: function(form) {
			var ph = $('#email').val()+','+$('#phoneNumber').val();
			$('input[name="phone"]').val(ph);

			var mi = 'Facebook ID(전화번호) : '+$('input[name="messengerId"]').val();
	...
	function formValid() {
		...
		if ($('#q_form input:checkbox[name="agree"]:checked').val()) {
			...
			$('#username-error').text('Đồng ý với các điều khoản và điều kiện để sử dụng dịch vụ.').show(); //서비스 이용을 위해 약관에 동의해주세요. 수정해야 될 부분
		}
	...
	function tooltip(mode) {
		if(mode == 'plus') {
			$('.tooltip').text('Đã lưu dịch vụ');	//시술이 담겼습니다.   수정해야 될 부분
			$('.tooltip').fadeIn(250);
			setTimeout(function(){ $('.tooltip').fadeOut(400); }, 2000);
		} else {
			$('.tooltip').text('Hủy lựa chọn ');	//선택 취소되었습니다.  수정해야 될 부분
			$('.tooltip').fadeIn(250);
			setTimeout(function(){ $('.tooltip').fadeOut(400); }, 2000);
		}
	}
	...
	function optSelect(target, txt, price, status) {
		...
		if(!price) price = 'Đăng kí thăm khám/tư vấn tại viện';		//내원 후 상담처럼 금액이 없는 경우
		...
		if(status == 'plus') {	//장바구니 추가
			...
			optSel+= '<div class="opt-item" data-target="'+target+'">'
				...
				//+'<em>'+price+'</em></p>'
				+ (
					(price == 'Đăng kí thăm khám/tư vấn tại viện') ?
		...
		if(checkCnt == 0) {
			cartTxt = 'Vui lòng lựa sản phẩm dự kiến thực hiện ';	//시술 받으실 상품을 선택해주세요
			$('.totalSmBox .priceBox').removeClass('on');
		} else {
			cartTxt = 'Đặt trước ';	//예약
			$('.totalSmBox .priceBox').addClass('on');
		}
		$('.cart_view').text(cartTxt);
		$('.btn-cart > span').text(checkCnt);

		if(price != 'Đăng kí thăm khám/tư vấn tại viện') cartTotalPrice(optPrice, status);
	...
	$(document).on('click', '.totalSmInner', function (e) {
		...
		if(chkCnt == 0) {
			...
			e.preventDefault();
			alert('Vui lòng lựa chọn dịch vụ muốn đặt trước ');	//예약하실 시술을 먼저 선택해주세요!  수정해야 될 부분
	...
	$('.btnConsultClose').click(function(e) {
		...
		$('.reserveTimeWrapper .sectionTitle em').text('Lựa chọn thời gian ').removeClass('on'); // 시간 선택
	...
	function searchKey() {
		...
		if(!findKey) {
			...
			alert('Nhập từ tìm kiếm '); //검색어를 입력해주세요.
			...
			if(findKeyCnt == 0) {
				...
				alert('Không có dịch vụ phù hợp. \\nVui lòng tìm kiếm và lựa chọn lại. '); //일치하는 시술이 없습니다. \n다시 검색해 주시기 바랍니다. 수정해야 될 부분
				$('.searchForm input').focus();
	...
	function detailResultFunc(r){
		...
		$('.itemViewBox .noti > em').text('Lưu ý');	//주의사항

		$('.itemViewBox .infoArea dt:contains("마취시간")').text('Thời gian tiêm tê.gây mê');
		$('.itemViewBox .infoArea dt:contains("시술시간")').text('Thời gian thực hiện');
		$('.itemViewBox .infoArea dt:contains("회복기간")').text('Thời gian hồi phục');
		$('.itemViewBox .infoArea dt:contains("유지기간")').text('Thời gian duy trì');
		$('.itemViewBox .infoArea dt:contains("재시술주기")').text('Chu kì thực hiện');
	}
