$(function() {
	...
	$('.btn_add').text('เลือกบริการ');	//시술 담기
	$('.btn_view').text('ดูเพิ่มเติม');	//상세 보기

	$('.hipassSwitch').html('รวดเร็ว ไม่รอคิว/รับคำปรึกษา! <em>ดูบริการ HIPASS</em>');	//대기/상담없이 빠르게! 하이패스 시술보기
	...
	var validator = $( "#q_form" ).validate({
		...
		messages: {
			tblStrField2: "เลือกวันที่ต้องการจอง",	//희망 예약일을 선택해주세요.
			tblStrField3: "เลือกเวลาที่ต้องการจอง", //희망 예약시간을 선택해주세요.
			select: "지점을 선택해주세요",
			name: "กรอกชื่อผู้รับบริการ", //이름을 입력해주세요
			country: "ระบุสัญชาติ",	//국적을 입력해주세요.
			etcDirectOpt: "바로예약 상담 항목을 선택해주세요.",
			phoneNumber: {
				required: "กรอกเบอร์โทรศัพท์", //연락처를 입력해주세요
				minlength: "กรอกเบอร์โทรศัพท์",
				maxlength: "กรอกเบอร์โทรศัพท์"
			},
			email: {
				required: "กรอกที่อยู่อีเมล",
				email: "กรุณากรอกที่อยู่อีเมลให้ถูกต้อง"
			},
			messengerId: "กรอก LINE ID หรือเบอร์โทรศัพท์ที่สามารถติดต่อได้",	//LINE 계정을 검색할 수 있는 ID 혹은 전화번호를 입력해주세요
			tblStrField: "진료구분을 선택해주세요.",
			agree: "กรุณายอมรับเงื่อนไขเพื่อรับบริการ", //서비스 이용을 위해 약관에 동의해주세요.
			smspush: "กรุณายอมรับเงื่อนไขเพื่อรับบริการ"
		},
		submitHandler: function(form) {
			var ph = $('#email').val()+','+$('#phoneNumber').val();
			$('input[name="phone"]').val(ph);

			var mi = 'LINE ID(전화번호) : '+$('input[name="messengerId"]').val();
	...
	function formValid() {
		...
		if ($('#q_form input:checkbox[name="agree"]:checked').val()) {
			...
			$('#username-error').text('กรุณายอมรับเงื่อนไขเพื่อรับบริการ').show(); //서비스 이용을 위해 약관에 동의해주세요.
		}
		if ($('#q_form input:checkbox[name="smspush"]:checked').val()) {
			vaildItem += 1;
		}
		else {
			$('#username-error').text('กรุณายอมรับเงื่อนไขเพื่อรับบริการ').show();
		}
	...
	function tooltip(mode) {
		if(mode == 'plus') {
			$('.tooltip').text('บันทึกบริการแล้ว');	//시술이 담겼습니다.
			$('.tooltip').fadeIn(250);
			setTimeout(function(){ $('.tooltip').fadeOut(400); }, 2000);
		} else {
			$('.tooltip').text('ยกเลิกการเลือกแล้ว');	//선택 취소되었습니다.
			$('.tooltip').fadeIn(250);
			setTimeout(function(){ $('.tooltip').fadeOut(400); }, 2000);
		}
	}
	...
	function optSelect(target, txt, price, status) {
		...
		if(checkCnt == 0) {
			cartTxt = 'เลือกบริการ';	//시술 받으실 상품을 선택해주세요
			$('.totalSmBox .priceBox').removeClass('on');
		} else {
			cartTxt = 'จองคิว';	//이벤트 신청하기
	...
	$(document).on('click', '.totalSmInner', function (e) {
		...
		if(chkCnt == 0) {
			...
			e.preventDefault();
			alert('เลือกบริการ');	//예약하실 시술을 먼저 선택해주세요!
	...
	$('.btnConsultClose').click(function(e) {
		...
		$('.reserveTimeWrapper .sectionTitle em').text('เลือกเวลา').removeClass('on')		//시간 선택;
	...
	function searchKey() {
		...
		if(!findKey) {
			...
			alert('กรุณากรอกคำค้นหา'); //검색어를 입력해주세요.
			...
			if(findKeyCnt == 0) {
				...
				alert('ไม่บริการที่ท่านค้นหา กรุณา'); //일치하는 시술이 없습니다. \n다시 검색해 주시기 바랍니다.
				$('.searchForm input').focus();
	...
	function detailResultFunc(r){
		...
		$('.itemViewBox .noti > em').text('Post-treatment Instructions');	//주의사항

		$('.itemViewBox .infoArea dt:contains("마취시간")').text('Anesthetic');
		$('.itemViewBox .infoArea dt:contains("시술시간")').text('Treatment time');
		$('.itemViewBox .infoArea dt:contains("회복기간")').text('Recovery time');
		$('.itemViewBox .infoArea dt:contains("유지기간")').text('Effect duration');
		$('.itemViewBox .infoArea dt:contains("재시술주기")').text('Follow-up treatment');
	}
