$(function() {
	...
	$('.btn_add').text('Add to cart');	//시술 담기
	$('.btn_view').text('Learn more');	//상세 보기

	$('.hipassSwitch').html('No more waiting! <em>Get Hi-Pass!</em>');	//대기/상담없이 빠르게! 하이패스 시술보기
	...
	var validator = $( "#q_form" ).validate({
		...
		messages: {
			tblStrField2: "Select desired date of appointment",	//희망 예약일을 선택해주세요.
			tblStrField3: "Select desired time of appointment", //희망 예약시간을 선택해주세요.
			select: "지점을 선택해주세요",
			name: "Input your name", //이름을 입력해주세요.
			country: "Please state your nationality",	//국적을 입력해주세요.
			etcDirectOpt: "바로예약 상담 항목을 선택해주세요.",
			phoneNumber: {
				required: "Please enter a valid phone number", //연락처를 입력해주세요
			...
			messengerId: "Input your WhatsApp account",	//와츠앱 계정을 검색할 수 있는 전화번호를 입력해주세요
			tblStrField: "진료구분을 선택해주세요.",
			agree: "Agree to service policy", //서비스 이용을 위해 약관에 동의해주세요.
			smspush: "Agree to service policy"
		},
	...
	function formValid() {
		...
		if ($('#q_form input:checkbox[name="agree"]:checked').val()) {
			...
			$('#username-error').text('Agree to service policy').show(); //서비스 이용을 위해 약관에 동의해주세요.
		}
		if ($('#q_form input:checkbox[name="smspush"]:checked').val()) {
			vaildItem += 1;
		}
		else {
			$('#username-error').text('Agree to service policy').show();
		}
	...
	function tooltip(mode) {
		if(mode == 'plus') {
			$('.tooltip').text('Added to cart');	//시술이 담겼습니다.
			$('.tooltip').fadeIn(250);
			setTimeout(function(){ $('.tooltip').fadeOut(400); }, 2000);
		} else {
			$('.tooltip').text('Selection is cancelled');	//선택 취소되었습니다.
			$('.tooltip').fadeIn(250);
			setTimeout(function(){ $('.tooltip').fadeOut(400); }, 2000);
		}
	}
	...
	function optSelect(target, txt, price, status) {
		...
		if(checkCnt == 0) {
			cartTxt = 'Choose desired treatment';	//시술 받으실 상품을 선택해주세요
			$('.totalSmBox .priceBox').removeClass('on');
		} else {
			cartTxt = 'Make an reservation';	//이벤트 신청하기
	...
	$(document).on('click', '.totalSmInner', function (e) {
		...
		if(chkCnt == 0) {
			...
			e.preventDefault();
			alert('Please select your desired procedure');	//예약하실 시술을 먼저 선택해주세요!
	...
	function searchKey() {
		...
		if(!findKey) {
			...
			if(findKeyCnt == 0) {
				...
				alert('Procedure does not exist.\nPlease search again.'); //일치하는 시술이 없습니다. \n다시 검색해 주시기 바랍니다.
				$('.searchForm input').focus();
