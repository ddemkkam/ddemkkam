$(function() {
	...
	$('.btn_add').text('施術をカートに入れる');	//시술 담기
	$('.btn_view').text('詳細を見る');	//상세 보기

	$('.hipassSwitch').html('待機、相談なしに早く! <em>ハイパス施術を見る</em>');	//대기/상담없이 빠르게! 하이패스 시술보기
	...
	//$("#q_form").validate({
		...
		messages: {
			tblStrField2: "希望の予約日を選択してください。",	//희망 예약일을 선택해주세요.
			tblStrField3: "希望の予約時間を選択してください。", //희망 예약시간을 선택해주세요.
			select: "지점을 선택해주세요",
			name: "お名前を入力してください。", //이름을 입력해주세요.
			country: "国籍を入力してください",	//국적을 입력해주세요.
			etcDirectOpt: "바로예약 상담 항목을 선택해주세요.",
			phoneNumber: {
				required: "携帯電話を入力してください。", //연락처를 입력해주세요
				minlength: "携帯電話を入力してください。",
				maxlength: "携帯電話を入力してください。"
			},
			email: {
				required: "メールアドレスを入力してください。",
				email: "正確なメールアドレスを入力してください。"
			},
			messengerId: "LINEアカウントを検索できるIDもしくは電話番号を入力してください。",
			tblStrField: "진료구분을 선택해주세요.",
			agree: "サービス利用のため約款に同意してください。", //서비스 이용을 위해 약관에 동의해주세요.
			smspush: "サービス利用のため約款に同意してください。"
		},
	...
	/*if($('#select option:selected').val() != 'ppeum16') {		//명동점 제외
	...
	function formValid() {
		...
		if ($('#q_form input:checkbox[name="agree"]:checked').val()) {
			...
			$('#username-error').text('サービス利用のため約款に同意してください。').show(); //서비스 이용을 위해 약관에 동의해주세요.
		}
		if ($('#q_form input:checkbox[name="smspush"]:checked').val()) {
			vaildItem += 1;
	    }
		else {
			$('#username-error').text('サービス利用のため約款に同意してください。').show();
		}
	...
	function tooltip(mode) {
		if(mode == 'plus') {
			$('.tooltip').text('施術がかごに入りました。');	//시술이 담겼습니다.
			$('.tooltip').fadeIn(250);
			setTimeout(function(){ $('.tooltip').fadeOut(400); }, 2000);
		} else {
			$('.tooltip').text('選択が取り消されました。');	//선택 취소되었습니다.
			$('.tooltip').fadeIn(250);
			setTimeout(function(){ $('.tooltip').fadeOut(400); }, 2000);
		}
	}
	...
	function optSelect(target, txt, price, status) {
		...
		if(checkCnt == 0) {
			cartTxt = '施術を希望される商品を選んでください';	//시술 받으실 상품을 선택해주세요
			$('.totalSmBox .priceBox').removeClass('on');
		} else {
			cartTxt = '予約する';	//이벤트 신청하기
	...
	$(document).on('click', '.totalSmInner', function (e) {
		...
		if(chkCnt == 0) {
			...
			e.preventDefault();
			alert('先にご予約予定の施術を選択してください。');	//예약하실 시술을 먼저 선택해주세요!
	...
	$('.btnConsultClose').click(function(e) {
		...
		$('.reserveTimeWrapper .sectionTitle em').text('時間選択').removeClass('on');
	...
	function searchKey() {
		...
		if(!findKey) {
			...
			alert('検索ワードを入力してください'); //검색어를 입력해주세요.
	...
	function detailResultFunc(r){
		...
		$('.itemViewBox .infoArea dt:contains("마취시간")').text('麻酔時間');
		$('.itemViewBox .infoArea dt:contains("시술시간")').text('施術時間');
		$('.itemViewBox .infoArea dt:contains("회복기간")').text('回復時間');
		$('.itemViewBox .infoArea dt:contains("유지기간")').text('維持期間');
		$('.itemViewBox .infoArea dt:contains("재시술주기")').text('再施術周期');
	}
