$(function() {
	...
	$('.btn_add').text('시술 담기');	//시술 담기 수정해야 될 부분
	$('.btn_view').text('상세보기');	//상세 보기 수정해야 될 부분

	$('.hipassSwitch').html('대기/상담없이 빠르게! <em>하이패스 시술보기</em>');	//대기/상담없이 빠르게! 하이패스 시술보기 수정해야 될 부분
	...
	var validator = $( "#q_form" ).validate({
		...
		messages: {
			tblStrField2: "희망 예약일을 선택해주세요.",	//희망 예약일을 선택해주세요. 수정해야 될 부분
			tblStrField3: "희망 예약시간을 선택해주세요.", //희망 예약시간을 선택해주세요. 수정해야 될 부분
			select: "지점을 선택해주세요",
			name: "이름을 입력해주세요.", //이름을 입력해주세요. 수정해야 될 부분
			etcDirectOpt: "바로예약 상담 항목을 선택해주세요.",
			phoneNumber: {
				required: "연락처를 입력해주세요", //연락처를 입력해주세요 수정해야 될 부분
				minlength: "연락처를 입력해주세요",
				maxlength: "연락처를 입력해주세요"
			},
			email: {
				required: "メールアドレスを入力してください。", //이메일 주소를 입력하세요  수정해야 될 부분
				email: "正確なメールアドレスを入力してください。" //정확한 이메일 주소를 입력하세요. 수정해야 될 부분
			},
			tblStrField: "진료구분을 선택해주세요.",
			agree: "서비스 이용을 위해 약관에 동의해주세요.", //서비스 이용을 위해 약관에 동의해주세요.  수정해야 될 부분
			smspush: "서비스 이용을 위해 약관에 동의하십시오." // 서비스 이용을 위해 약관에 동의하십시오. 수정해야 될 부분
		},
	...
	function formValid() {
		...
		if ($('#q_form input:checkbox[name="agree"]:checked').val()) {
			...
			$('#username-error').text('서비스 이용을 위해 약관에 동의해주세요.').show(); //서비스 이용을 위해 약관에 동의해주세요. 수정해야 될 부분
		}
		if ($('#q_form input:checkbox[name="smspush"]:checked').val()) {
			vaildItem += 1;
		}
		else {
			$('#username-error').text('サービス利用のため約款に同意してください。').show(); //서비스 이용을 위해 약관에 동의해주세요.  수정해야 될 부분
		}

		if ($('#q_form input:radio[name="contactInfo"]:checked').val() == 'E') {
			if ($('#q_form #email').val() && !$('#q_form #email').hasClass('error')) {
				vaildItem += 1;
			}
		}
		else if ($('#q_form input:radio[name="contactInfo"]:checked').val() == 'P') {
	...
	function tooltip(mode) {
		if(mode == 'plus') {
			$('.tooltip').text('시술이 담겼습니다.');	//시술이 담겼습니다.   수정해야 될 부분
			$('.tooltip').fadeIn(250);
			setTimeout(function(){ $('.tooltip').fadeOut(400); }, 2000);
		} else {
			$('.tooltip').text('선택 취소되었습니다.');	//선택 취소되었습니다.  수정해야 될 부분
			$('.tooltip').fadeIn(250);
			setTimeout(function(){ $('.tooltip').fadeOut(400); }, 2000);
		}
	}
	...
	function optSelect(target, txt, price, status) {
		...
		if(checkCnt == 0) {
			cartTxt = '시술 받으실 상품을 선택해주세요';	//시술 받으실 상품을 선택해주세요  수정해야 될 부분
			$('.totalSmBox .priceBox').removeClass('on');
		} else {
			cartTxt = '예약';	//이벤트 신청하기 수정해야 될 부분
	...
	$(document).on('click', '.totalSmInner', function (e) {
		...
		if(chkCnt == 0) {
			...
			e.preventDefault();
			alert('예약하실 시술을 먼저 선택해주세요!');	//예약하실 시술을 먼저 선택해주세요!  수정해야 될 부분
	...
	$('.btnConsultClose').click(function(e) {
		...
		$('.reserveTimeWrapper .sectionTitle em').text('시간 선택').removeClass('on'); // 시간 선택 수정해야 될 부분
	...
	function searchKey() {
		...
		if(!findKey) {
			...
			alert('검색어를 입력해주세요.'); //검색어를 입력해주세요. 수정해야 될 부분
	...
	function detailResultFunc(r){
		...
		$('.itemViewBox .infoArea dt:contains("마취시간")').text('마취시간'); //수정해야 될 부분
		$('.itemViewBox .infoArea dt:contains("시술시간")').text('시술시간'); //수정해야 될 부분
		$('.itemViewBox .infoArea dt:contains("회복기간")').text('회복기간'); //수정해야 될 부분
		$('.itemViewBox .infoArea dt:contains("유지기간")').text('유지기간'); //수정해야 될 부분
		$('.itemViewBox .infoArea dt:contains("재시술주기")').text('재시술주기'); //수정해야 될 부분
	}
