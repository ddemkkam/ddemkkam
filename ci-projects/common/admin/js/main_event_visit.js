$(function() {
	...
	$(document).on('click','.event_tab_cont li:not(.none_hipass) .btn_add',function(e) {
		...
		if($(this).closest('li').hasClass('selected')) {
			...
			optPrice = $('.cartBox .opt-item[data-target="' + optId + '"]').attr('data-item-price');		//상품단가 또는 이벤트단가
			optQty = $('.cartBox .opt-item[data-target="' + optId + '"]').find('.opt-count-box input').val();		//상품 선택 수량
			optTotal = $.number(optPrice*optQty);	//상품단가 * 선택 수량

			//옵션 장바구니에서 빼기
			optSelect(optId, optName, optTotal, 'minus');
	...
	//장바구니 수량 더하기
	$(document).on('click','.opt-item .qty-plus',function() {
		$(this).prev().val(+$(this).prev().val() + 1);

		//시술금액 합계 전달
		var price = $(this).closest('.opt-item').attr('data-item-price');
		cartTotalPrice(price, 'plus');
	});

	//장바구니 수량 빼기
	$(document).on('click','.opt-item .qty-minus',function() {
		if ($(this).next().val() > 1) {
			$(this).next().val(+$(this).next().val() - 1);

			//시술금액 합계 전달
			var price = $(this).closest('.opt-item').attr('data-item-price');
			cartTotalPrice(price, 'minus');
		}
	});
	...
	function optSelect(target, txt, price, status) {
		...
		if(status == 'plus') {	//장바구니 추가
			...
			optSel+= '<div class="opt-item" data-target="'+target+'" data-item-price="'+optPrice+'">'
			+'<p><span>'+txt+'</span>'
			+ (
			(price == '내원 후 상담') ?
			'	<em class="txt">'+price+'</em></p>'
			:	//일반 또는 이벤트 상품
			'	<b class="opt-count-box"><button type="button" class="qty-minus">감소</button><input type="number" name="qty[]" value="1" size="2" readonly><button type="button" class="qty-plus">증가</button></b><em>'+price+'</em></p>'
			)
	...
	$(document).on('click','.cartBox .opt-item .del',function(e) {
		e.preventDefault();
		var optId='', optName='', optPrice = 0, optPrice2 = 0, optQty = 1, optTotal = 0;
		...
		optSelect(optId, optName, optTotal, 'delete');
	});
